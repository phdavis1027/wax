...
//! demonstrates how the `wax::trace` module can be used to instrument `wax`
...
async fn main() {
    ...
    let filter = std::env::var("RUST_LOG").unwrap_or_else(|_| "tracing=info,wax=debug".to_owned());
    ...
    let goodbye = wax::domain_is("goodbye")
        .and(wax::get())
        .map(|| {
